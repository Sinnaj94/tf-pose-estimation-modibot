...
class ModibotMetadata:
    ...
    def get_heatmap(self, target_size):
        heatmap = np.zeros((ModibotMetadata.__modibot_parts, self.height, self.width), dtype=np.float32)

        for joints in self.joint_list:
            for idx, point in enumerate(joints):
                if point[0] < 0 or point[1] < 0:
                    continue
                ModibotMetadata.put_heatmap(heatmap, idx, point, self.sigma)
