...
TRAINING_DIR = "/Users/jannis/Nextcloud/Dokumente/Hochschule/Semester_5/software/mobi_synth"
...
def get_dataflow(path, is_train):
    ds = ModiBot(path, is_train)       # read data from lmdb
    if is_train:
...
if __name__ == '__main__':
    ...
    df = _get_dataflow_onlyread(TRAINING_DIR, True)
    ...
    with tf.compat.v1.Session() as sess:
        ...
        for idx, dp in enumerate(df.get_data()):
            ...
            print(time.time() - t1)
            t1 = time.time()
            ModiBot.display_image(dp[0], dp[1].astype(np.float32))
            pass
